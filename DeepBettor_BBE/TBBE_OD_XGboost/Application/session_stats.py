...
def getXGboostTrainData(trades, simId, bettingAgents, agentDistances):
    balances = getBalance(bettingAgents)
    agentDistances['rank'] = 
agentDistances.groupby('time')['distance'].rank(ascending=False, 
method='first').astype(int)
    new_header = ["type", "competitorID", "time", "exchange", "odds", 
"agentID", "stake", "distance", "rank", "balance", "decision"]
    tape = []
...
(abs(agentDistances['time'] - time) < tolerance)
        filtered_df = agentDistances[mask]
        distance = filtered_df['distance'].values[0] if len(filtered_df) > 
0 else 0 
        rank = filtered_df['rank'].values[0] if len(filtered_df) > 0 else 
0 
        backer_balance = balances[val["backer"]]
        backer_row = [val["type"], val["competitor"], val["time"], 
val["exchange"], val["odds"], val["backer"], val["stake"], distance, rank, 
backer_balance, "backer"]
        tape.append(backer_row)
        layer_balance = balances[val["layer"]]
        layer_row = [val["type"], val["competitor"], val["time"], 
val["exchange"], val["odds"], val["layer"], val["stake"], distance, rank, 
layer_balance, "layer"]
        tape.append(layer_row)
    ...
    fileName = os.path.join(output_dir, "getXGBOOstTrainingData_" + 
str(simId) + ".csv")
    with open(fileName, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(new_header)
